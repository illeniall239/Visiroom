...
const TextPressure = ({
  ...
  useEffect(() => {
    ...
    const debouncedSetSize = (...args: Parameters<typeof setSize>) => {
      clearTimeout(timeoutId);
      timeoutId = setTimeout(() => setSize(...args), 100);
    };
    debouncedSetSize();
  ...
  const dynClass = ["tp-title", className, flex ? "tp-flex" : "", stroke ? "tp-stroke" : ""]
    .filter(Boolean)
    .join(" ");
